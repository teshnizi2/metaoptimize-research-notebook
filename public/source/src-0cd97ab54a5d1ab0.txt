...
From the command line:
  ...
  --check  every listed (run, job_id) is present EXACTLY once in results/all_runs.csv; no key is listed
           ...
           DECAY-ROUTE SWITCH (CORRECTIONS 308): DECAY_ROUTE (patches/patch_decayroute.py, 305) is a tenth line kind -- its
           tree prints `DECAY_ROUTE: off` or ONE `DECAY_ROUTE: on mode=...` line on every run.  cai1's 32 runs (all at
           `--weight-decay-base 0`) and crd1's 6 AI* runs are TWO-AXIS (284's rule); crd1's 12 SR* / TR* runs are one-kind
           rows listed by the line.  Every listed run of those batches is also held to its ARM's registered line
           (DECAY_ROUTE_ARMS), so a row that copied a wrong mode or LAMBDA FAILs;
...
# ---- CORRECTIONS 308: DECAY_ROUTE, the tenth kind (patches/patch_decayroute.py, CORRECTIONS 305; cai1 306, crd1 307) --------
# `DECAY_ROUTE=shrink_only | trace_only | alpha_indep:<LAMBDA>` changes the route of the base weight decay (the weights'
# shrink, the meta-trace's factor, or both made alpha-independent), which no CSV column carries; its tree ($WS/harness_cdr1)
# prints `DECAY_ROUTE: off` or ONE `DECAY_ROUTE: on mode=<m> base=<alg> wd=<repr> lambda=<repr|na> lambda_f32=<repr|na>
# gamma=<repr>` line on every run (305.2).  Appended; the nine entries above are unchanged.  Keyed on the FULL prefix
# (305.6 d): DECAY_ROUTE and DECAY_MASK share `DECAY_` and diverge at index 6 (`R` / `M`), so neither is a prefix of the
# other -- the necessary and sufficient condition check() proves over ALL of KINDS.  As for every kind, witness_lines takes
# a line that STARTS WITH the prefix, so an echo such as `DECAY_ROUTE=...` would be collected as a second, ON-looking
# line and FAIL a listed run loudly (fail-closed); the harness prints none (305.5, the proof log's real runs).
# tests/test_corpus_exclusions_check.py C46-C50.
KINDS.append(("DECAY_ROUTE", "DECAY_ROUTE: off"))
KINDS_AT_304 = 9   # CORRECTIONS 308: 304's `kinds scanned` line is frozen over the first nine entries, so it stays byte-identical
...
# CORRECTIONS 308: cai1 (306) and crd1 (307), 284's TWO-AXIS rule applied to DECAY_ROUTE.  PATCH_DECAYROUTE refuses
# alpha_indep at any `--weight-decay-base` but 0 (it REPLACES the alpha-scaled decay), so EVERY alpha_indep run also deviates
# on ARGS_WD_BASE: cai1's 32 runs (both rungs, all grains) and crd1's AIS / AIL are listed with their ARGS witness
# `ARGS_WD_BASE: weight-decay-base=0` and their DECAY_ROUTE line is registered here.  crd1's SR* / TR* run at the standard 0.1
# and are ONE-kind rows listed by their DECAY_ROUTE line; they register nothing here (they are in DECAY_ROUTE_ARMS below).
# So crd1 owes 18 rows, not 24: one (run, job_id) is listed ONCE (the duplicate-key check), and 307.9's "18 rows plus 6
# ARGS_WD_BASE rows" means 12 + 6 two-axis rows.  Re-typed (NOT imported) from the registered designs
# `analysis/cai1_design.py` `witness_on(rung)` and `analysis/crd1_design.py` `dr_witness(arm)`, which print the harness's
# own format; the I5 and shrink_only / trace_only strings are byte-identical to the lines proof job 5081090's REAL runs
# printed (305.5 RR2).  tests/test_corpus_exclusions_check.py C49 / C50 pin them.
_DR_CAI1_I5 = ("DECAY_ROUTE: on mode=alpha_indep base=SGDm wd=0.0 lambda=5e-05 lambda_f32=4.999999873689376e-05 "
               "gamma=1.0")
_DR_CAI1_I4 = ("DECAY_ROUTE: on mode=alpha_indep base=SGDm wd=0.0 lambda=0.0005 lambda_f32=0.0005000000237487257 "
               "gamma=1.0")
_DR_CRD1_SR = "DECAY_ROUTE: on mode=shrink_only base=SGDm wd=0.1 lambda=na lambda_f32=na gamma=1.0"
_DR_CRD1_TR = "DECAY_ROUTE: on mode=trace_only base=SGDm wd=0.1 lambda=na lambda_f32=na gamma=1.0"
_DR_CRD1_AI = ("DECAY_ROUTE: on mode=alpha_indep base=SGDm wd=0.0 lambda=0.000315 lambda_f32=0.0003150000120513141 "
               "gamma=1.0")
# Every registered route arm and its ONE line.  check() holds every LISTED run of a batch named here to its arm's line,
# whichever witness the row carries -- so a row whose own witness copied a wrong mode or LAMBDA still FAILs, and a listed
# run of such a batch whose arm is not here FAILs.  Unlisted runs are untouched (the batch rule holds them to `off`).
DECAY_ROUTE_ARMS = {}
for _g in ("ch", "nd", "k01", "kL"):
    DECAY_ROUTE_ARMS[("cai1", _g + "I5")] = _DR_CAI1_I5
    DECAY_ROUTE_ARMS[("cai1", _g + "I4")] = _DR_CAI1_I4
DECAY_ROUTE_ARMS.update({
    ("crd1", "SRS"): _DR_CRD1_SR, ("crd1", "SRL"): _DR_CRD1_SR,
    ("crd1", "TRS"): _DR_CRD1_TR, ("crd1", "TRL"): _DR_CRD1_TR,
    ("crd1", "AIS"): _DR_CRD1_AI, ("crd1", "AIL"): _DR_CRD1_AI,
})
DECAY_ROUTE_TWO_AXIS = [k for k in sorted(DECAY_ROUTE_ARMS) if k[0] == "cai1"] + [("crd1", "AIS"), ("crd1", "AIL")]
MULTI_KIND.update(dict((k, {"DECAY_ROUTE": DECAY_ROUTE_ARMS[k]}) for k in DECAY_ROUTE_TWO_AXIS))
...
def check(runs_dirs):
    ...
    if runs_dirs:
        ...
        # CORRECTIONS 308: 304's line is frozen over the NINE kinds it registered (KINDS_AT_304), as 269's is over eight;
        # the kind added since is named on the next line.
        print("  kinds scanned (CORRECTIONS 304): also %s, %d in all; no prefix of the %d is a prefix of another: %s"
              % (" / ".join(kd for kd, _off in KINDS[KINDS_AT_269:KINDS_AT_304]), KINDS_AT_304, KINDS_AT_304, not collide))
        print("  kinds scanned (CORRECTIONS 308): also %s, %d in all; no prefix of the %d is a prefix of another: %s"
              % (" / ".join(kd for kd, _off in KINDS[KINDS_AT_304:]), len(KINDS), len(KINDS), not collide))
        ...
        # ---- DECAY-ROUTE ARMS (CORRECTIONS 308) --------------------------------------------------------------------
        # Every listed run of a batch in DECAY_ROUTE_ARMS prints exactly its ARM's registered line, whichever witness its
        # row carries (the DECAY_ROUTE line itself, or the ARGS witness of a two-axis run); a listed run of such a batch
        # whose arm is not registered FAILs.  Re-checked here independently of the readers above, so the one-kind rows'
        # mode / LAMBDA are held to the registration and not only to the TSV row.
        nb = len(bad)
        n_dr = 0
        drb = set(b for b, _a in DECAY_ROUTE_ARMS)
        for fn in sorted(listed):
            ba = tuple(fn.split("-")[:2])
            if ba[0] not in drb:
                continue
            if ba not in DECAY_ROUTE_ARMS:
                bad.append("%s is a listed run of the DECAY_ROUTE batch %s but its arm %s has no registered line"
                           " (DECAY_ROUTE_ARMS)" % (fn, ba[0], ba[1] if len(ba) > 1 else "?"))
                continue
            if fn not in wl:
                continue
            n_dr += 1
            if wl[fn]["DECAY_ROUTE"] != [DECAY_ROUTE_ARMS[ba]]:
                bad.append("%s is a registered DECAY_ROUTE arm but prints %r, not its arm's registered line"
                           " (DECAY_ROUTE_ARMS)" % (fn, wl[fn]["DECAY_ROUTE"]))
        print("  decay-route runs (CORRECTIONS 308): %d listed runs of a registered route arm (DECAY_ROUTE_ARMS), every one"
              " printing exactly its arm's registered DECAY_ROUTE line, and no listed run of a route batch outside the"
              " registry: %s" % (n_dr, len(bad) == nb))
    ...
    # CORRECTIONS 308: DECAY_ROUTE_ARMS is well-formed -- every line a `DECAY_ROUTE: on mode=` line -- and every DECAY_ROUTE
    # line MULTI_KIND registers for a route arm is that arm's line.  Checked with or without --runs; prints nothing, only FAILs.
    for (mb, ma), w in sorted(DECAY_ROUTE_ARMS.items()):
        if kind_of(w) != "DECAY_ROUTE" or not w.startswith("DECAY_ROUTE: on mode="):
            bad.append("DECAY_ROUTE_ARMS entry %s/%s: %r is not a well-formed DECAY_ROUTE ON line" % (mb, ma, w))
    for (mb, ma), des in sorted(MULTI_KIND.items()):
        if "DECAY_ROUTE" in des and des["DECAY_ROUTE"] != DECAY_ROUTE_ARMS.get((mb, ma)):
            bad.append("MULTI_KIND entry %s/%s: its DECAY_ROUTE line %r disagrees with DECAY_ROUTE_ARMS (%r)"
                       % (mb, ma, des["DECAY_ROUTE"], DECAY_ROUTE_ARMS.get((mb, ma))))
