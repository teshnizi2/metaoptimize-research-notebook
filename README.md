...
The snapshot currently covers 158 records — 140 research questions and 18 method checks, including the 37-row count-matched partition audit from MASTER-TABLE section 10, the six rows appended at MASTER-TABLE lines 212-217 (MT212-MT217), the cvt1 row at line 218 (MT218), the cgn3 row at line 219 (MT219) and the cvt3 and cvt2 rows at lines 220-221 (MT220-MT221), with the in-place row amendments of CORRECTIONS 229, 231, 234 and 236 — plus 3,019 recorded runs, 54 report figures, and the completed CVK2 cut-location test. Coverage and publication metadata come from `public/data/research.json`. Job completion is separate from scientific success, and each conclusion retains its comparison and scope.
...
- All 3,019 runs in this snapshot have an included sanitized raw log, linked by `logHref`, with original/public SHA-256 pairs. The package report records the exact published-log count. If a future snapshot has an archived-but-unpublished log, its ledger entry must retain that explicit availability distinction; an unpublished log is different from a nonexistent log.
