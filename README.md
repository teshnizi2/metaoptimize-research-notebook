...
- All 3,268 runs in this snapshot have an included sanitized raw log, linked by `logHref`, with original/public SHA-256 pairs. The package report records the exact published-log count. If a future snapshot has an archived-but-unpublished log, its ledger entry must retain that explicit availability distinction; an unpublished log is different from a nonexistent log.
