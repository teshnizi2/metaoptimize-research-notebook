...
The snapshot currently covers 172 records — 154 research questions and 18 method checks, including the 37-row count-matched partition audit from MASTER-TABLE section 10, the six rows appended at MASTER-TABLE lines 212-217 (MT212-MT217), the cvt1 row at line 218 (MT218), the cgn3 row at line 219 (MT219), the cvt3 and cvt2 rows at lines 220-221 (MT220-MT221) the cvt4 and cvt5 rows at lines 222-223 (MT222-MT223), the cvt6 and cvt7 rows at lines 224-225 (MT224-MT225), the cvt8 and cvt9 rows at lines 226-227 (MT226-MT227), the four MUST-tier rows at lines 228-231 (MT228-MT231: cmo1, cst1, cct1, cmg1) and the four mechanism rows at lines 232-235 (MT232-MT235: cvt10, cwd1, csv1, cwd2), with the in-place row amendments of CORRECTIONS 229, 231, 234, 236, 244, 253.15 and 273 — plus 3,253 recorded runs, 54 report figures, and the completed CVK2 cut-location test. Coverage and publication metadata come from `public/data/research.json`. Job completion is separate from scientific success, and each conclusion retains its comparison and scope.
...
- All 3,253 runs in this snapshot have an included sanitized raw log, linked by `logHref`, with original/public SHA-256 pairs. The package report records the exact published-log count. If a future snapshot has an archived-but-unpublished log, its ledger entry must retain that explicit availability distinction; an unpublished log is different from a nonexistent log.
