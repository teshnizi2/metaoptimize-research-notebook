...
def warning_for(experiment):
    ...
    for further in experiment.get("further_amendments") or []:
        # Later in-place amendments (CORRECTIONS 234, 236, 244; 253.15 keyed as 253; 273) each keep their own record, keyed on the
        # entry number. CORRECTIONS 273 is the first of them to move an outcome (MT229, Open -> Mixed), so the warning says so and
        # names the outcome the record carried before, exactly as the CORRECTIONS 229 amendment record does.
        moved = further.get("previousOutcome") != further.get("outcome")
        detail = further["reason"]
        if moved:
            detail += f" Outcome before the amendment: {register_model.OUTCOME_LABELS[further['previousOutcome']]}."
        warnings.append({"id": f"warning-{experiment['id']}-amendment-{further['number']}", "experimentId": experiment["id"], "severity": "limitation",
                         "title": "Outcome moved by a later result" if moved else "Wording amended by a later result", "status": "documented",
                         "detail": detail + " Amendment record: " + further["source"] + "."})
...
def make_runs(inventory, experiments, evidence, public, workspace, intervened=None):
    ...
    for row in inventory:
        ...
        if job_id in intervened and intervened[job_id]["argsDeviation"]:
            ...
            axes = register_model.args_extra_kinds(listed["intervention"])
            ...
            if axes:
                # A TWO-AXIS row (CORRECTIONS 284): the run deviates on an ARGS value AND runs a patch, so one witness
                # column cannot describe it. The patch axes are named here and witnessed from the run's own log below.
                params["argsDeviationAxes"] = " + ".join(f"{patch}={patch_value}" for patch, patch_value in axes)
            ...
            # (CORRECTIONS 245, 251).
            params["interventionNote"] = (f"{register_model.intervention_opening(listed['looks_like'])}: the inventory carries that arm's cell key because no column records the "
        ...
        if listed and listed["argsDeviation"]:
            ...
            extra_holds = register_model.args_extra_kinds(listed["intervention"])
        else:
            extra_holds = register_model.intervention_kinds(listed["intervention"])[1:] if listed else []
        if extra_holds:
...
def validate(data, runs, public):
    ...
    for line, (rule, section, batches, figures, _, _) in register_model.MUST_ROWS.items():
        eid = register_model.partition_id(line)
        row = by_id.get(eid, {})
        # MT229 is the one row of that landing a later one amended: CORRECTIONS 273 moved it from Open to Mixed, so its
        # outcome is the amendment's, not MUST_ROWS'. Every other row must still carry the outcome its own rule gives.
        expected_outcome = register_model.MECH4_AMENDMENTS.get(eid, {}).get("outcome", register_model.RULE_OUTCOME[rule])
        verify(row.get("section") == section and row.get("area") == register_model.AREAS[section] and row.get("outcome") == expected_outcome, f"MUST-tier MASTER-TABLE row missing or remapped: line {line}")
    ...
    # The MUST-tier landings amended no row themselves; the only amendment warning on any of the four is the one a LATER
    # landing (CORRECTIONS 273) put on MT229, and it must be exactly that one.
    verify(sorted(w["id"] for w in data["warnings"] if w["id"].startswith(tuple(f"warning-{eid}-amendment" for eid in MUST_IDS)))
           == sorted(f"warning-{eid}-amendment-{spec['number']}" for eid, spec in register_model.MECH4_AMENDMENTS.items() if eid in MUST_IDS),
           "The MUST-tier landings amended no row of their own")
    for line, (rule, section, batches, figures, _, _) in register_model.MECH4_ROWS.items():
        eid = register_model.partition_id(line)
        row = by_id.get(eid, {})
        verify(row.get("section") == section and row.get("area") == register_model.AREAS[section] and row.get("outcome") == register_model.RULE_OUTCOME[rule], f"cvt10/cwd1/csv1/cwd2 MASTER-TABLE row missing or remapped: line {line}")
        verify(row.get("batches") == batches and set(figures) <= set(row.get("figureIds", [])), f"cvt10/cwd1/csv1/cwd2 row links: line {line}")
        verify(any(e["kind"] == "research-phase" and eid in e["experimentIds"] for e in data["activity"]), f"cvt10/cwd1/csv1/cwd2 row has no research phase: line {line}")
        verify(not row.get("corrected"), f"cvt10/cwd1/csv1/cwd2 row must carry no Corrected badge: line {line}")
        linked = [run for run in runs if eid in run["experimentIds"]]
        verify(bool(linked) and {run["batch"] for run in linked} == set(batches), f"cvt10/cwd1/csv1/cwd2 row runs not linked: {eid}")
    verify(not any(w["id"].startswith(tuple(f"warning-{eid}-registration" for eid in MECH4_IDS)) for w in data["warnings"]), "The cvt10/cwd1/csv1/cwd2 landing corrected no registration text")
    verify(not any(w["id"].startswith(tuple(f"warning-{eid}-amendment" for eid in MECH4_IDS)) for w in data["warnings"]), "The cvt10/cwd1/csv1/cwd2 landing amended none of its own rows")
    for line, (rule, section, batches, figures, _, _) in register_model.MECH5_ROWS.items():
        eid = register_model.partition_id(line)
        row = by_id.get(eid, {})
        verify(row.get("section") == section and row.get("area") == register_model.AREAS[section] and row.get("outcome") == register_model.RULE_OUTCOME[rule], f"cwd3 MASTER-TABLE row missing or remapped: line {line}")
        verify(row.get("batches") == batches and set(figures) <= set(row.get("figureIds", [])), f"cwd3 row links: line {line}")
        verify(any(e["kind"] == "research-phase" and eid in e["experimentIds"] for e in data["activity"]), f"cwd3 row has no research phase: line {line}")
        verify(not row.get("corrected"), f"cwd3 row must carry no Corrected badge: line {line}")
        linked = [run for run in runs if eid in run["experimentIds"]]
        verify(bool(linked) and {run["batch"] for run in linked} == set(batches), f"cwd3 row runs not linked: {eid}")
    # The cwd3 landing amended NO row of its own and none of anybody else's: it is the first landing since the MUST tier
    # to move no outcome at all, so no amendment warning may name it and none may carry its entry number.
    verify(not any(w["id"].startswith(tuple(f"warning-{eid}-registration" for eid in MECH5_IDS)) for w in data["warnings"]), "The cwd3 landing corrected no registration text")
    verify(not any(w["id"].startswith(tuple(f"warning-{eid}-amendment" for eid in MECH5_IDS)) for w in data["warnings"]), "The cwd3 landing amended none of its own rows")
    verify(not any(w["id"].endswith("-amendment-278") for w in data["warnings"]), "The cwd3 landing amended no earlier row either")
    for line, (rule, section, batches, figures, _, _) in register_model.MECH6_ROWS.items():
        eid = register_model.partition_id(line)
        row = by_id.get(eid, {})
        verify(row.get("section") == section and row.get("area") == register_model.AREAS[section] and row.get("outcome") == register_model.RULE_OUTCOME[rule], f"cwd4/cwd5 MASTER-TABLE row missing or remapped: line {line}")
        verify(row.get("batches") == batches and set(figures) <= set(row.get("figureIds", [])), f"cwd4/cwd5 row links: line {line}")
        verify(any(e["kind"] == "research-phase" and eid in e["experimentIds"] for e in data["activity"]), f"cwd4/cwd5 row has no research phase: line {line}")
        verify(not row.get("corrected"), f"cwd4/cwd5 row must carry no Corrected badge: line {line}")
        linked = [run for run in runs if eid in run["experimentIds"]]
        verify(bool(linked) and {run["batch"] for run in linked} == set(batches), f"cwd4/cwd5 row runs not linked: {eid}")
    # Neither landing amended a row of its own or anybody else's, so no amendment warning may name either record or carry
    # either entry number, and neither corrected any registration text.
    verify(not any(w["id"].startswith(tuple(f"warning-{eid}-registration" for eid in MECH6_IDS)) for w in data["warnings"]), "The cwd4/cwd5 landings corrected no registration text")
    verify(not any(w["id"].startswith(tuple(f"warning-{eid}-amendment" for eid in MECH6_IDS)) for w in data["warnings"]), "The cwd4/cwd5 landings amended none of their own rows")
    verify(not any(w["id"].endswith(("-amendment-283", "-amendment-285")) for w in data["warnings"]), "The cwd4/cwd5 landings amended no earlier row either")
    # cwd4 owns patch-intervention rows and no ARGS-value row; cwd5 owns ARGS-value rows and no patch-intervention row,
    # three of them TWO-AXIS (CORRECTIONS 284), which are the first rows in the corpus to carry both.
    verify(register_model.partition_id(register_model.MECH6_STEP_ROW) in register_model.MECH6_INTERVENTIONS
           and register_model.partition_id(register_model.MECH6_LAST_ROW) in register_model.MECH6_ARGS_DEVIATIONS,
    ...
    for line, (rule, section, batches, figures, _, _) in register_model.MECH8_ROWS.items():
        eid = register_model.partition_id(line)
        row = by_id.get(eid, {})
        verify(row.get("section") == section and row.get("area") == register_model.AREAS[section] and row.get("outcome") == register_model.RULE_OUTCOME[rule], f"crt1/csh1 MASTER-TABLE row missing or remapped: line {line}")
        verify(row.get("batches") == batches and set(figures) <= set(row.get("figureIds", [])), f"crt1/csh1 row links: line {line}")
        verify(any(e["kind"] == "research-phase" and eid in e["experimentIds"] for e in data["activity"]), f"crt1/csh1 row has no research phase: line {line}")
        verify(not row.get("corrected"), f"crt1/csh1 row must carry no Corrected badge: line {line}")
        linked = [run for run in runs if eid in run["experimentIds"]]
        verify(bool(linked) and {run["batch"] for run in linked} == set(batches), f"crt1/csh1 row runs not linked: {eid}")
    # Neither landing amended a row of its own or anybody else's (MT240 keeps its record: row 240 was not amended), and
    # neither corrected any registration text.
    verify(not any(w["id"].startswith(tuple(f"warning-{eid}-registration" for eid in MECH8_IDS)) for w in data["warnings"]), "The crt1/csh1 landings corrected no registration text")
    verify(not any(w["id"].startswith(tuple(f"warning-{eid}-amendment" for eid in MECH8_IDS)) for w in data["warnings"]), "The crt1/csh1 landings amended none of their own rows")
    verify(not any(w["id"].endswith(tuple(f"-amendment-{n}" for n in register_model.MECH8_ENTRIES)) for w in data["warnings"]), "The crt1/csh1 landings amended no earlier row either")
    # Both own ARGS-value rows only, one kind per run: every one of the 54 runs is listed by its base weight decay 5e-4.
    verify(set(register_model.MECH8_ARGS_DEVIATIONS) == set(MECH8_IDS) and not register_model.MECH8_INTERVENTIONS, "The crt1 and csh1 rows are ARGS-value deviations only")
    for run in runs:
        if run["batch"] in ("crt1", "csh1"):
            verify(run["parameters"].get("argsDeviationWitness") == "ARGS_WD_BASE: weight-decay-base=5e-4" and not run["parameters"].get("argsDeviationAdditionalArgs")
                   and not run["parameters"].get("argsDeviationAxes"), f"crt1/csh1 run is not a one-kind ARGS_WD_BASE row: {run['id']}")
    for line, (rule, section, batches, figures, _, _) in register_model.MECH9_ROWS.items():
        eid = register_model.partition_id(line)
        row = by_id.get(eid, {})
        verify(row.get("section") == section and row.get("area") == register_model.AREAS[section] and row.get("outcome") == register_model.RULE_OUTCOME[rule], f"cvl1 MASTER-TABLE row missing or remapped: line {line}")
        verify(row.get("batches") == batches and set(figures) <= set(row.get("figureIds", [])), f"cvl1 row links: line {line}")
        verify(any(e["kind"] == "research-phase" and eid in e["experimentIds"] for e in data["activity"]), f"cvl1 row has no research phase: line {line}")
        verify(not row.get("corrected"), f"cvl1 row must carry no Corrected badge: line {line}")
        linked = [run for run in runs if eid in run["experimentIds"]]
        verify(bool(linked) and {run["batch"] for run in linked} == set(batches), f"cvl1 row runs not linked: {eid}")
    # The landing amended no row of its own or anybody else's (MT240 keeps its record), and corrected no registration text.
    verify(not any(w["id"].startswith(tuple(f"warning-{eid}-registration" for eid in MECH9_IDS)) for w in data["warnings"]), "The cvl1 landing corrected no registration text")
    verify(not any(w["id"].startswith(tuple(f"warning-{eid}-amendment" for eid in MECH9_IDS)) for w in data["warnings"]), "The cvl1 landing amended none of its own rows")
    verify(not any(w["id"].endswith(tuple(f"-amendment-{n}" for n in register_model.MECH9_ENTRIES)) for w in data["warnings"]), "The cvl1 landing amended no earlier row either")
    # cvl1 owns BOTH kinds of row in ONE batch: its 16 W1 runs are one-kind VAL_SPLIT patch rows, its 16 W4 runs two-axis
    # ARGS_WD_BASE rows with the VAL_SPLIT clause read back from each run's own log.
    verify(set(register_model.MECH9_ARGS_DEVIATIONS) == set(MECH9_IDS) == set(register_model.MECH9_INTERVENTIONS), "The cvl1 row owns both VAL_SPLIT and two-axis ARGS rows")
    for run in runs:
        if run["batch"] == "cvl1":
            if run["parameters"]["runLabel"].split("-")[1].endswith("W1"):
                verify(run["parameters"].get("intervention", "").endswith(": VAL_SPLIT=5000:302") and run["parameters"].get("interventionWitness", "").startswith("VAL_SPLIT: on ")
                       and "argsDeviation" not in run["parameters"], f"cvl1 W1 run is not a one-kind VAL_SPLIT row: {run['id']}")
            else:
                verify(run["parameters"].get("argsDeviationWitness") == "ARGS_WD_BASE: weight-decay-base=5e-4" and run["parameters"].get("argsDeviationAxes") == "VAL_SPLIT=5000:302"
                       and not run["parameters"].get("argsDeviationAdditionalArgs"), f"cvl1 W4 run is not a two-axis ARGS_WD_BASE + VAL_SPLIT row: {run['id']}")
    for line, (rule, section, batches, figures, _, _) in register_model.MECH10_ROWS.items():
        eid = register_model.partition_id(line)
        row = by_id.get(eid, {})
        verify(row.get("section") == section and row.get("area") == register_model.AREAS[section] and row.get("outcome") == register_model.RULE_OUTCOME[rule], f"g3b MASTER-TABLE row missing or remapped: line {line}")
        verify(row.get("batches") == batches and set(figures) <= set(row.get("figureIds", [])), f"g3b row links: line {line}")
        verify(any(e["kind"] == "research-phase" and eid in e["experimentIds"] for e in data["activity"]), f"g3b row has no research phase: line {line}")
        verify(not row.get("corrected"), f"g3b row must carry no Corrected badge: line {line}")
        linked = [run for run in runs if eid in run["experimentIds"]]
        verify(bool(linked) and {run["batch"] for run in linked} == set(batches), f"g3b row runs not linked: {eid}")
    # The landing amended no row of its own or anybody else's (MT240 and MT155 keep their records), and corrected no
    # registration text.
    verify(not any(w["id"].startswith(tuple(f"warning-{eid}-registration" for eid in MECH10_IDS)) for w in data["warnings"]), "The g3b landing corrected no registration text")
    verify(not any(w["id"].startswith(tuple(f"warning-{eid}-amendment" for eid in MECH10_IDS)) for w in data["warnings"]), "The g3b landing amended none of its own rows")
    verify(not any(w["id"].endswith(tuple(f"-amendment-{n}" for n in register_model.MECH10_ENTRIES)) for w in data["warnings"]), "The g3b landing amended no earlier row either")
    # g3b owns ARGS-value rows only, one kind per run: its 16 W4 runs are listed by their base weight decay 5e-4; its 16 W1
    # runs sit at the standard 0.1 and are not marked.
    verify(set(register_model.MECH10_ARGS_DEVIATIONS) == set(MECH10_IDS) and not register_model.MECH10_INTERVENTIONS, "The g3b row is an ARGS-value deviation only")
    for run in runs:
        if run["batch"] == "g3b":
            if run["parameters"]["runLabel"].split("-")[1].endswith("W4"):
                verify(run["parameters"].get("argsDeviationWitness") == "ARGS_WD_BASE: weight-decay-base=5e-4" and not run["parameters"].get("argsDeviationAdditionalArgs")
    ...
    for eid, spec in register_model.MECH4_AMENDMENTS.items():
        ...
        verify(row.get("outcome") == spec["outcome"] and not row.get("corrected"), f"CORRECTIONS {spec['entry']} amendment outcome or badge wrong: {eid}")
        verify(warning.get("experimentId") == eid and warning.get("id") in row.get("warningIds", []), f"CORRECTIONS {spec['entry']} amendment warning missing: {eid}")
        verify(f"line {spec['line']} at {register_model.MECH4_COMMIT[:7]}; CORRECTIONS {spec['entry']}" in warning.get("detail", ""), f"CORRECTIONS {spec['entry']} amendment record missing: {eid}")
        verify(register_model.OUTCOME_LABELS[spec["previous"]] in warning.get("detail", "") and warning.get("title") == "Outcome moved by a later result", f"CORRECTIONS {spec['entry']} must name the outcome it moved: {eid}")
        verify(spec["branch"] in row.get("reason", "") and spec["superseded"] in row.get("reason", ""), f"CORRECTIONS {spec['entry']} must keep the superseded verdict: {eid}")
