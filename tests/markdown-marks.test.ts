...
test('every record keeps its ID, section, area, kind, outcome and Corrected badge; a title changes only by losing marks', () => {
  ...
  // MT224-MT235 (cvt6, cvt7, cvt8, cvt9, the four MUST-tier batches, then cvt10, cwd1, csv1 and cwd2) were appended after the
  // fix (tests/cvt6-cvt7-landing.test.ts, tests/cvt8-cvt9-landing.test.ts, tests/must-tier-landing.test.ts, tests/mech4-landing.test.ts).
  assert.deepEqual(data.experiments.slice(0, 160).map(e => e.id), fixture.experiments.map(e => e.id));
  assert.deepEqual(data.experiments.slice(160).map(e => e.id), ['MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235']);
