...
test('MT239 and MT240 carry the outcome the documented rules give them, with every FINAL token quoted', () => {
  assert.deepEqual(data.experiments.slice(-2).map(e => e.id), ['MT239', 'MT240'], 'appended after every existing record');
...
test('the 67 runs link to their records with sanitized logs, and exactly the 46 listed rows are marked', () => {
  ...
  assert.equal(runs.filter(run => run.parameters.argsDeviation).length, 85, "cmo1's 18, cwd5's 21, caw2's 18 and cgw1's 28");
});
