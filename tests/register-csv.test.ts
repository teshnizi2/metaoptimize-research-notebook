...
test('the register CSV has one row per published record and the documented columns', () => {
  ...
  assert.deepEqual([table.rows, table.columns], [168, columns.length], 'the table catalog reports the regenerated size');
});
...
test('every CSV row carries its four-way outcome, kind and a separate Corrected column', () => {
  ...
  assert.deepEqual([count('kind', 'research'), count('kind', 'method-check'), count('corrected', 'Corrected')], [150, 18, 35]);
  assert.deepEqual(['success', 'fail', 'mixed', 'unresolved'].map(outcome => count('outcome', outcome)), [51, 41, 34, 24]);
