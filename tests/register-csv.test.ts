...
test('the register CSV has one row per published record and the documented columns', () => {
  ...
  assert.deepEqual([table.rows, table.columns], [156, columns.length], 'the table catalog reports the regenerated size');
});
...
test('every CSV row carries its four-way outcome, kind and a separate Corrected column', () => {
  ...
  assert.deepEqual([count('kind', 'research'), count('kind', 'method-check'), count('corrected', 'Corrected')], [138, 18, 35]);
  assert.deepEqual(['success', 'fail', 'mixed', 'unresolved'].map(outcome => count('outcome', outcome)), [47, 41, 27, 23]);
  ...
  assert.deepEqual(rows.filter(row => /^MT21[2-9]$/.test(row.id)).map(row => [row.id, row.master_table_line, row.outcome_label]),
    [['MT212', '212', 'Goal met'], ['MT213', '213', 'Mixed'], ['MT214', '214', 'Goal met'], ['MT215', '215', 'Goal met'], ['MT216', '216', 'Goal met'], ['MT217', '217', 'Goal met'], ['MT218', '218', 'Mixed'], ['MT219', '219', 'Goal met']]);
});
