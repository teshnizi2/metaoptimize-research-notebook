...
test('MT237 and MT238 carry the outcome the documented rules give them, with every FINAL token quoted', () => {
  ...
  assert.deepEqual(data.experiments.slice(-4, -2).map(e => e.id), ['MT237', 'MT238'], 'appended in line order, before the later caw2 and cgw1 rows');
...
test('the 48 runs link to their records with sanitized logs, and exactly the listed rows are marked', () => {
  ...
  assert.equal(runs.filter(run => run.parameters.intervention).length, 183, 'the 165 earlier patch interventions plus cwd3 and cwd4');
  assert.equal(runs.filter(run => !['caw2', 'cgw1'].includes(run.batch) && run.parameters.argsDeviation).length, 39, "cmo1's 18 plus cwd5's 21");
  // caw2 and cgw1 later added 18 and 28 of their own (tests/mech7-landing.test.ts).
  assert.equal(runs.filter(run => run.parameters.argsDeviation).length, 85);
});

test("cwd5's CARW2 rows are the corpus's first two-axis rows and are witnessed on both axes", () => {
  const twoAxis = runs.filter(run => run.parameters.argsDeviationAxes);
...
test('phase-13 and phase-14 are their own documented phases and carry one record each', () => {
  ...
  // phase-15 and phase-16 were added later with caw2 and cgw1 (tests/mech7-landing.test.ts).
  assert.equal(data.activity.filter(e => e.kind === 'research-phase').length, 16);
});
