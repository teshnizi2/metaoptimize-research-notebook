...
test('the 36 runs link to MT220 / MT221 with sanitized logs, and exactly the 24 intervened runs are marked', () => {
  ...
  assert.equal(runs.filter(run => run.parameters.intervention).length, 153, 'cvt1 9 + cvt3 9 + cvt2 15 + cvt4 12 + cvt5 6 + cvt6 15 + cvt7 9 + cvt8 15 + cvt9 18 + cvt10 15 + cwd1 6 + csv1 12 + cwd2 12');
