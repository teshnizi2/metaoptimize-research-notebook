...
test('the published register is fully represented in the general review', async () => {
  ...
  assert.equal(rows.reduce((sum, row) => sum + row.total, 0), 140);
  ...
  // MASTER-TABLE lines 212-217: five mechanism rows and one baseline row (cuc1); line 218 (cvt1, Mixed) is a sixth mechanism row, line 219 (cgn3, Goal met) a seventh, and lines 220-221 (cvt3, cvt2, both Mixed) the eighth and ninth.
  assert.deepEqual([rows.find(row => row.label === 'Mechanism and isolation')!.total, rows.find(row => row.label === 'Mechanism and isolation')!.counts], [44, { success: 13, fail: 12, mixed: 11, unresolved: 8 }]);
