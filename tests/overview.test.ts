...
test('the published register is fully represented in the general review', async () => {
  ...
  assert.equal(rows.reduce((sum, row) => sum + row.total, 0), 155);
  ...
  assert.deepEqual([rows.find(row => row.label === 'Mechanism and isolation')!.total, rows.find(row => row.label === 'Mechanism and isolation')!.counts], [59, { success: 22, fail: 12, mixed: 17, unresolved: 8 }]);
