...
test('the published register is fully represented in the general review', async () => {
  ...
  assert.equal(rows.reduce((sum, row) => sum + row.total, 0), 163);
  ...
  assert.deepEqual([audit.total, audit.counts, audit.methodChecks], [38, { success: 12, fail: 5, mixed: 5, unresolved: 16 }, 3]);
  // MASTER-TABLE line 243 (cvl1, Open) is the audit area's thirty-seventh research question (tests/mech9-landing.test.ts), and
  // line 244 (g3b, Open) its thirty-eighth (tests/mech10-landing.test.ts).
