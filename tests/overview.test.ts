...
test('the published register is fully represented in the general review', async () => {
  ...
  assert.equal(rows.reduce((sum, row) => sum + row.total, 0), 146);
  ...
  assert.deepEqual([rows.find(row => row.label === 'Mechanism and isolation')!.total, rows.find(row => row.label === 'Mechanism and isolation')!.counts], [50, { success: 16, fail: 12, mixed: 14, unresolved: 8 }]);
