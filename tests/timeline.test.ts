...
test('newest-first timeline retains documented windows and every original CSV observation', async () => {
  ...
  // phase-13 and phase-14 share a date (the cwd5 landing is 2026-09-20 22:41 UTC), so the newest-first sort keeps them in register order.
  assert.deepEqual(rows.map(row => row.id), ['phase-13', 'phase-14', 'phase-12', 'phase-11', 'phase-10', ...Array.from({ length: 9 }, (_, i) => `phase-0${9 - i}`)]);
  assert.equal(csv.length, 8, 'the campaign timeline CSV is published unchanged');
  // phase-09 through phase-14 are added by the notebook register (scripts/register_model.py ADDED_PHASES), not by the CSV.
  rows.filter(row => !['phase-09', 'phase-10', 'phase-11', 'phase-12', 'phase-13', 'phase-14'].includes(row.id)).forEach(row => {
...
test('coverage deduplicates overlaps without dating unlinked records from the snapshot import', async () => {
  ...
  assert.equal(rows.reduce((sum, row) => sum + row.experiments.length, 0), 96);
  assert.equal(phaseIds.size, 95);
