...
test('coverage deduplicates overlaps without dating unlinked records from the snapshot import', async () => {
  ...
  assert.equal(rows.reduce((sum, row) => sum + row.experiments.length, 0), 76);
  assert.equal(phaseIds.size, 75);
...
test('the 15-16 Sep phase links exactly the rows appended at MASTER-TABLE lines 212-218', async () => {
  const { researchTimeline, filterByResearchPhase } = await helpers();
  ...
  const ids = ['MT212', 'MT213', 'MT214', 'MT215', 'MT216', 'MT217', 'MT218'];
  ...
  assert.deepEqual(row.counts, { success: 5, fail: 0, mixed: 2, unresolved: 0 });
