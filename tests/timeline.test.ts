...
test('newest-first timeline retains documented windows and every original CSV observation', async () => {
  ...
  assert.deepEqual(rows.map(row => row.id), ['phase-12', 'phase-11', 'phase-10', ...Array.from({ length: 9 }, (_, i) => `phase-0${9 - i}`)]);
  assert.equal(csv.length, 8, 'the campaign timeline CSV is published unchanged');
  // phase-09 through phase-12 are added by the notebook register (scripts/register_model.py ADDED_PHASES), not by the CSV.
  rows.filter(row => !['phase-09', 'phase-10', 'phase-11', 'phase-12'].includes(row.id)).forEach(row => {
...
test('coverage deduplicates overlaps without dating unlinked records from the snapshot import', async () => {
  ...
  assert.equal(rows.reduce((sum, row) => sum + row.experiments.length, 0), 94);
  assert.equal(phaseIds.size, 93);
