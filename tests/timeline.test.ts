...
test('newest-first timeline retains documented windows and every original CSV observation', async () => {
  ...
  // cgw1, both landed on 2026-09-22), so the newest-first sort keeps each pair in register order.
  assert.deepEqual(rows.map(row => row.id), ['phase-15', 'phase-16', 'phase-13', 'phase-14', 'phase-12', 'phase-11', 'phase-10', ...Array.from({ length: 9 }, (_, i) => `phase-0${9 - i}`)]);
  assert.equal(csv.length, 8, 'the campaign timeline CSV is published unchanged');
  // phase-09 through phase-16 are added by the notebook register (scripts/register_model.py ADDED_PHASES), not by the CSV.
  rows.filter(row => !['phase-09', 'phase-10', 'phase-11', 'phase-12', 'phase-13', 'phase-14', 'phase-15', 'phase-16'].includes(row.id)).forEach(row => {
...
test('coverage deduplicates overlaps without dating unlinked records from the snapshot import', async () => {
  ...
  assert.equal(rows.reduce((sum, row) => sum + row.experiments.length, 0), 98);
  assert.equal(phaseIds.size, 97);
