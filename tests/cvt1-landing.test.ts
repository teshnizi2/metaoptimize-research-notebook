...
test('the 15 cvt1 runs link to MT218, and exactly the 9 intervened runs are marked with their VOTE_W witness', () => {
  ...
  assert.deepEqual([...new Set(runs.filter(run => run.parameters.intervention).map(run => run.batch))].sort(), ['csv1', 'cvt1', 'cvt10', 'cvt2', 'cvt3', 'cvt4', 'cvt5', 'cvt6', 'cvt7', 'cvt8', 'cvt9', 'cwd1', 'cwd2', 'cwd3']);
});
