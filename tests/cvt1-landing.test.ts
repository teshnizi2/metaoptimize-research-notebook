...
test('the 15 cvt1 runs link to MT218, and exactly the 9 intervened runs are marked with their VOTE_W witness', () => {
  ...
  // cvt3 and cvt2 add their own 9 and 15 (tests/cvt3-cvt2-landing.test.ts); no other batch is marked.
  assert.deepEqual([...new Set(runs.filter(run => run.parameters.intervention).map(run => run.batch))].sort(), ['cvt1', 'cvt2', 'cvt3']);
});
