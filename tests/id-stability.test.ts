...
test('only the 37 partition-audit rows, the 6 appended rows, the cvt1 row, the cgn3 row, the cvt3 and cvt2 rows, the cvt4 and cvt5 rows, the cvt6 and cvt7 rows, the cvt8 and cvt9 rows and the four MUST-tier rows are new, keyed on their line in the pinned MASTER-TABLE', () => {
  ...
  assert.deepEqual(added.sort(), [...partitionIds, ...appendedIds, 'MT218', 'MT219', 'MT220', 'MT221', 'MT222', 'MT223', 'MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240'].sort());
...
test('MASTER-TABLE anchors resolve by row content, not by the old line numbers', () => {
  ...
  for (const id of [...partitionIds, ...appendedIds, 'MT218', 'MT219', 'MT220', 'MT221', 'MT222', 'MT223', 'MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240']) assert.equal(lineOf(id), Number(id.slice(2)));
});
...
test('all 154 records published before the line-218 import keep their ID, title, area, kind and outcome; only MT218 is added', () => {
  ...
  assert.deepEqual(added, ['MT218', 'MT219', 'MT220', 'MT221', 'MT222', 'MT223', 'MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240']);
  assert.equal(data.experiments.length, 177);
});
...
test('all 155 records published before the line-219 import keep their ID, title, area, kind and outcome; only MT219 is added', () => {
  ...
  assert.deepEqual(added, ['MT219', 'MT220', 'MT221', 'MT222', 'MT223', 'MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240']);
  assert.equal(data.experiments.length, 177);
});
...
test('all 156 records published before the lines 220-221 import keep their ID, title, area, kind and outcome; only MT220 and MT221 are added', () => {
  ...
  assert.deepEqual(added, ['MT220', 'MT221', 'MT222', 'MT223', 'MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240']);
  assert.equal(data.experiments.length, 177);
});
...
test('all 158 records published before the lines 222-223 import keep their ID, title, area, kind and outcome; only MT222 and MT223 are added', () => {
  ...
  assert.deepEqual(added, ['MT222', 'MT223', 'MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240']);
  assert.equal(data.experiments.length, 177);
});
...
test('all 160 records published before the lines 224-225 import keep their ID, title, area, kind and outcome; only MT224 and MT225 are added', () => {
  ...
  assert.deepEqual(added, ['MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240']);
  assert.equal(data.experiments.length, 177);
});
...
test('all 172 records published at 98b0e8d keep their ID, section, area, title, kind, outcome, Corrected badge and order; only MT236, MT237 and MT238 are added', () => {
  ...
  assert.deepEqual(added, ['MT236', 'MT237', 'MT238', 'MT239', 'MT240']);
...
test('all 173 records published at eddc014 keep their ID, section, area, title, kind, outcome, Corrected badge and order; only MT237-MT240 are added', () => {
  ...
  assert.deepEqual(added, ['MT237', 'MT238', 'MT239', 'MT240']);
...
test('all 175 records published at 429e0c2 keep their ID, section, area, title, kind, outcome, Corrected badge and order; only MT239 and MT240 are added', () => {
  ...
  assert.deepEqual(added, ['MT239', 'MT240']);
  // The two new records and their outcomes, pinned so a later import cannot move them silently.
  assert.deepEqual(data.experiments.slice(175).map(e => [e.id, e.batches, e.area, e.kind, e.outcome, e.corrected]), [
