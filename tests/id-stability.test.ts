...
test('only the 37 partition-audit rows, the 6 appended rows, the cvt1 row, the cgn3 row, the cvt3 and cvt2 rows, the cvt4 and cvt5 rows, the cvt6 and cvt7 rows, the cvt8 and cvt9 rows and the four MUST-tier rows are new, keyed on their line in the pinned MASTER-TABLE', () => {
  ...
  assert.deepEqual(added.sort(), [...partitionIds, ...appendedIds, 'MT218', 'MT219', 'MT220', 'MT221', 'MT222', 'MT223', 'MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240', 'MT241', 'MT242', 'MT243', 'MT244'].sort());
...
test('MASTER-TABLE anchors resolve by row content, not by the old line numbers', () => {
  ...
  for (const id of [...partitionIds, ...appendedIds, 'MT218', 'MT219', 'MT220', 'MT221', 'MT222', 'MT223', 'MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240', 'MT241', 'MT242', 'MT243', 'MT244']) assert.equal(lineOf(id), Number(id.slice(2)));
});
...
test('all 154 records published before the line-218 import keep their ID, title, area, kind and outcome; only MT218 is added', () => {
  ...
  assert.deepEqual(added, ['MT218', 'MT219', 'MT220', 'MT221', 'MT222', 'MT223', 'MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240', 'MT241', 'MT242', 'MT243', 'MT244']);
  assert.equal(data.experiments.length, 181);
});
...
test('all 155 records published before the line-219 import keep their ID, title, area, kind and outcome; only MT219 is added', () => {
  ...
  assert.deepEqual(added, ['MT219', 'MT220', 'MT221', 'MT222', 'MT223', 'MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240', 'MT241', 'MT242', 'MT243', 'MT244']);
  assert.equal(data.experiments.length, 181);
});
...
test('all 156 records published before the lines 220-221 import keep their ID, title, area, kind and outcome; only MT220 and MT221 are added', () => {
  ...
  assert.deepEqual(added, ['MT220', 'MT221', 'MT222', 'MT223', 'MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240', 'MT241', 'MT242', 'MT243', 'MT244']);
  assert.equal(data.experiments.length, 181);
});
...
test('all 158 records published before the lines 222-223 import keep their ID, title, area, kind and outcome; only MT222 and MT223 are added', () => {
  ...
  assert.deepEqual(added, ['MT222', 'MT223', 'MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240', 'MT241', 'MT242', 'MT243', 'MT244']);
  assert.equal(data.experiments.length, 181);
});
...
test('all 160 records published before the lines 224-225 import keep their ID, title, area, kind and outcome; only MT224 and MT225 are added', () => {
  ...
  assert.deepEqual(added, ['MT224', 'MT225', 'MT226', 'MT227', 'MT228', 'MT229', 'MT230', 'MT231', 'MT232', 'MT233', 'MT234', 'MT235', 'MT236', 'MT237', 'MT238', 'MT239', 'MT240', 'MT241', 'MT242', 'MT243', 'MT244']);
  assert.equal(data.experiments.length, 181);
});
...
test('all 172 records published at 98b0e8d keep their ID, section, area, title, kind, outcome, Corrected badge and order; only MT236, MT237 and MT238 are added', () => {
  ...
  assert.deepEqual(added, ['MT236', 'MT237', 'MT238', 'MT239', 'MT240', 'MT241', 'MT242', 'MT243', 'MT244']);
...
test('all 173 records published at eddc014 keep their ID, section, area, title, kind, outcome, Corrected badge and order; only MT237-MT242 are added', () => {
  ...
  assert.deepEqual(added, ['MT237', 'MT238', 'MT239', 'MT240', 'MT241', 'MT242', 'MT243', 'MT244']);
  // The two new records and their outcomes, pinned so a later import cannot move them silently.
  assert.deepEqual(data.experiments.slice(173, 175).map(e => [e.id, e.batches, e.area, e.kind, e.outcome, e.corrected]), [
...
// The 175 records published at notebook commit 429e0c2, the published main this import started from. MASTER-TABLE lines
// 239 (caw2) and 240 (cgw1) were appended together at campaign commit 8a99001 (CORRECTIONS 295 and 296). That landing
// amended no row in place -- it edited only header line 3 -- so no earlier record may change its ID, section, area,
// title, kind, outcome, badge or order.
const before239 = JSON.parse(readFileSync(new URL('./fixtures/register-ids-429e0c2.json', import.meta.url), 'utf8')) as typeof previous;

test('all 175 records published at 429e0c2 keep their ID, section, area, title, kind, outcome, Corrected badge and order; only MT239-MT242 are added', () => {
  assert.equal(before239.experiments.length, 175);
  assert.deepEqual(before239.experiments.map(e => e.id), data.experiments.slice(0, 175).map(e => e.id), 'existing records keep their order');
  for (const old of before239.experiments) {
    const current = byId.get(old.id);
    assert.ok(current, `${old.id} must not disappear`);
    assert.deepEqual([current.section, current.area, current.title, current.kind, current.outcome, current.corrected],
      [old.section, old.area, old.title, old.kind, old.outcome, old.corrected], old.id);
  }
  // Like the cwd3, cwd4 and cwd5 landings before them, these two moved NO outcome anywhere.
  assert.deepEqual(before239.experiments.filter(old => byId.get(old.id)!.outcome !== old.outcome).map(old => old.id), []);
  assert.ok(!data.warnings.some(w => /-amendment-29[5-7]$/.test(w.id)), 'no row was amended at CORRECTIONS 295, 296 or 297');
  const added = data.experiments.map(e => e.id).filter(id => !before239.experiments.some(e => e.id === id));
  assert.deepEqual(added, ['MT239', 'MT240', 'MT241', 'MT242', 'MT243', 'MT244']);
  // The two new records and their outcomes, pinned so a later import cannot move them silently.
  assert.deepEqual(data.experiments.slice(175, 177).map(e => [e.id, e.batches, e.area, e.kind, e.outcome, e.corrected]), [
    ['MT239', ['caw2'], 'Mechanism and isolation', 'research', 'mixed', false],
    ['MT240', ['cgw1'], 'Count-matched partition audit', 'research', 'unresolved', false],
  ]);
  assert.equal(data.experiments.length, 181);
});

// The 177 records published at notebook commit 60dc6ba, the published main this import started from. MASTER-TABLE lines
// 241 (crt1) and 242 (csh1) were appended together at campaign commit a53bce1 (CORRECTIONS 309 and 310). That landing
// amended no row in place -- it edited only header line 3 -- so no earlier record may change its ID, section, area,
// title, kind, outcome, badge or order. crt1 weakens MT240's scalar reading to a tie, but row 240 was not amended, so
// MT240 keeps its outcome.
const before241 = JSON.parse(readFileSync(new URL('./fixtures/register-ids-60dc6ba.json', import.meta.url), 'utf8')) as typeof previous;

test('all 177 records published at 60dc6ba keep their ID, section, area, title, kind, outcome, Corrected badge and order; only MT241 and MT242 are added', () => {
  assert.equal(before241.experiments.length, 177);
  assert.deepEqual(before241.experiments.map(e => e.id), data.experiments.slice(0, 177).map(e => e.id), 'existing records keep their order');
  for (const old of before241.experiments) {
    const current = byId.get(old.id);
    assert.ok(current, `${old.id} must not disappear`);
    assert.deepEqual([current.section, current.area, current.title, current.kind, current.outcome, current.corrected],
      [old.section, old.area, old.title, old.kind, old.outcome, old.corrected], old.id);
  }
  // Like the cwd3-cgw1 landings before them, these two moved NO outcome anywhere, MT240's included.
  assert.deepEqual(before241.experiments.filter(old => byId.get(old.id)!.outcome !== old.outcome).map(old => old.id), []);
  assert.ok(!data.warnings.some(w => /-amendment-(29[89]|30\d|310)$/.test(w.id)), 'no row was amended at CORRECTIONS 298-310');
  const added = data.experiments.map(e => e.id).filter(id => !before241.experiments.some(e => e.id === id));
  assert.deepEqual(added, ['MT241', 'MT242', 'MT243', 'MT244']);
  // The two new records and their outcomes, pinned so a later import cannot move them silently.
  assert.deepEqual(data.experiments.slice(177, 179).map(e => [e.id, e.batches, e.area, e.kind, e.outcome, e.corrected]), [
    ['MT241', ['crt1'], 'Count-matched partition audit', 'research', 'mixed', false],
    ['MT242', ['csh1'], 'Mechanism and isolation', 'research', 'mixed', false],
  ]);
  assert.equal(data.experiments.length, 181);
});

// The 179 records published at notebook commit 1ec1906, the published main this import started from. MASTER-TABLE line 243
// (cvl1) was appended at campaign commit a3d422a (CORRECTIONS 312). That landing amended no row in place -- it edited only
// header line 3 -- so no earlier record may change its ID, section, area, title, kind, outcome, badge or order. cvl1 re-reads
// MT240's cell on a held-out split, but row 240 was not amended, so MT240 keeps its outcome.
const before243 = JSON.parse(readFileSync(new URL('./fixtures/register-ids-1ec1906.json', import.meta.url), 'utf8')) as typeof previous;

test('all 179 records published at 1ec1906 keep their ID, section, area, title, kind, outcome, Corrected badge and order; only MT243 is added', () => {
  assert.equal(before243.experiments.length, 179);
  assert.deepEqual(before243.experiments.map(e => e.id), data.experiments.slice(0, 179).map(e => e.id), 'existing records keep their order');
  for (const old of before243.experiments) {
    const current = byId.get(old.id);
    assert.ok(current, `${old.id} must not disappear`);
    assert.deepEqual([current.section, current.area, current.title, current.kind, current.outcome, current.corrected],
      [old.section, old.area, old.title, old.kind, old.outcome, old.corrected], old.id);
  }
  assert.deepEqual(before243.experiments.filter(old => byId.get(old.id)!.outcome !== old.outcome).map(old => old.id), []);
  assert.ok(!data.warnings.some(w => /-amendment-31[1-3]$/.test(w.id)), 'no row was amended at CORRECTIONS 311-313');
  const added = data.experiments.map(e => e.id).filter(id => !before243.experiments.some(e => e.id === id));
  assert.deepEqual(added, ['MT243', 'MT244'], 'MT244 (g3b) was appended later');
  assert.deepEqual(data.experiments.slice(179, 180).map(e => [e.id, e.batches, e.area, e.kind, e.outcome, e.corrected]), [
    ['MT243', ['cvl1'], 'Count-matched partition audit', 'research', 'unresolved', false],
  ]);
  assert.equal(data.experiments.length, 181);
});

// The 180 records published at notebook commit 11a8788, the published main this import started from. MASTER-TABLE line 244
// (g3b) was appended at campaign commit 12a4c5d (CORRECTIONS 315). That landing amended no row in place -- it edited only
// header line 3 -- so no earlier record may change its ID, section, area, title, kind, outcome, badge or order. g3b asks
// MT240's question on the audit's CIFAR-100 cell and re-runs MT155's denominator arm at 5e-4, but neither row was amended, so
// both keep their outcome.
const before244 = JSON.parse(readFileSync(new URL('./fixtures/register-ids-11a8788.json', import.meta.url), 'utf8')) as typeof previous;

test('all 180 records published at 11a8788 keep their ID, section, area, title, kind, outcome, Corrected badge and order; only MT244 is added', () => {
  assert.equal(before244.experiments.length, 180);
  assert.deepEqual(before244.experiments.map(e => e.id), data.experiments.slice(0, 180).map(e => e.id), 'existing records keep their order');
  for (const old of before244.experiments) {
    const current = byId.get(old.id);
    assert.ok(current, `${old.id} must not disappear`);
    assert.deepEqual([current.section, current.area, current.title, current.kind, current.outcome, current.corrected],
      [old.section, old.area, old.title, old.kind, old.outcome, old.corrected], old.id);
  }
  assert.deepEqual(before244.experiments.filter(old => byId.get(old.id)!.outcome !== old.outcome).map(old => old.id), []);
  assert.ok(!data.warnings.some(w => /-amendment-31[45]$/.test(w.id)), 'no row was amended at CORRECTIONS 314-315');
  const added = data.experiments.map(e => e.id).filter(id => !before244.experiments.some(e => e.id === id));
  assert.deepEqual(added, ['MT244']);
  assert.deepEqual(data.experiments.slice(180).map(e => [e.id, e.batches, e.area, e.kind, e.outcome, e.corrected]), [
    ['MT244', ['g3b'], 'Count-matched partition audit', 'research', 'unresolved', false],
  ]);
  assert.equal(data.experiments.length, 181);
});
