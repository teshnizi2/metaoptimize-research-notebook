...
test('MT226 (cvt8) and MT227 (cvt9) are both Mixed under the documented rules, with every FINAL token quoted', () => {
  assert.deepEqual(data.experiments.slice(-2).map(e => e.id), ['MT226', 'MT227'], 'appended after every existing record, in line order');
...
test('the 42 runs link to MT226 / MT227 with sanitized logs; the 33 intervened runs are marked, each further hold with its own witness', () => {
  ...
  assert.equal(runs.filter(run => run.parameters.intervention).length, 108, 'the 75 earlier + cvt8 15 + cvt9 18');
  assert.equal(runs.filter(run => run.parameters.interventionAdditionalWitness?.includes(' | ')).length, 6, 'EARLY and LATE carry two further holds');
...
test('the 75 earlier intervened runs keep their notes and witnesses byte for byte', () => {
  ...
  const earlier = runs.filter(run => !['cvt8', 'cvt9'].includes(run.batch) && run.parameters.intervention);
