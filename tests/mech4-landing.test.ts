...
test('MT232-MT235 carry the outcome the documented rules give them, with every FINAL token quoted', () => {
  assert.deepEqual(data.experiments.slice(-12, -8).map(e => e.id), ['MT232', 'MT233', 'MT234', 'MT235'], 'appended in line order, before the later cwd3, cwd4, cwd5, caw2, cgw1, crt1, csh1 and cvl1 rows');
