...
class ResearchExportTests(unittest.TestCase):
    ...
    def test_appended_rows_link_phase_figures_and_their_batch_runs(self):
        ...
        for run in runs:
            if run["batch"] in LANDED_BATCH_RUNS:
                ...
                expected_dataset = "CIFAR10" if run["batch"] in ("cct1", "cgw1", "crt1", "cvl1") else "CIFAR100"
    ...
    def test_only_cmo1_owes_exclusion_rows_and_they_carry_the_new_args_kinds(self):
        ...
        self.assertEqual(sum(not row["argsDeviation"] and row["batch"] != "cvl1" for row in intervened.values()), 183)
        with self.assertRaisesRegex(ValueError, "does not match the pinned bytes"):
    ...
    # ---- CORRECTIONS 312 (campaign commit a3d422a): MASTER-TABLE line 243, cvl1 ----

    @needs(_REPO)
    def test_mech9_rows_come_from_the_pinned_landing_commit(self):
        model = self.model()
        lines, before = model.mech9_master_table(REPO), model.mech8_master_table(REPO)
        self.assertEqual((len(before), len(lines)), (242, 243))
        # ONE step: header line 3 recounted and one row appended; no earlier row amended, line 5 untouched, no line moved.
        self.assertEqual({n for n in range(1, len(before) + 1) if lines[n - 1] != before[n - 1]}, {3})
        rows = model.mech9_rows(lines)
        self.assertEqual([(r["id"], r["section"], r["area"], r["outcome"], json.loads(r["batches"]), r["mapping_rule"], json.loads(r["figure_ids"]), r["corrected"]) for r in rows],
                         [("MT243", "10", "Count-matched partition audit", "unresolved", ["cvl1"], "open", ["page-8"], "")])
        self.assertTrue(rows[0]["reason"].startswith("Verdict: VAL-DIFFERS-UNRESOLVED + SELECT-SAME + W1-TUNDECIDED/VSURVIVES+W4-TUNDECIDED/VVANISHES+SC-TSCALAR-BEATS-BEST/VSCALAR-BEATS-BEST + "))
        self.assertIn("(primary + selection + per-claim states + 11 stamps; the 7 registered bounds in the next column) Open: VAL-DIFFERS-UNRESOLVED: ", rows[0]["reason"])
        # cvl1 owns BOTH kinds of exclusion row: the 16 W1 runs are one-kind VAL_SPLIT patch rows, the 16 W4 runs two-axis
        # ARGS rows; neither kind corrected any registration text.
        self.assertEqual([bool(r.get("intervention")) for r in rows], [True])
        self.assertEqual([bool(r.get("args_deviations")) for r in rows], [True])
        self.assertEqual([json.loads(r.get("registration_corrections") or "[]") for r in rows], [[]])
        with self.assertRaisesRegex(ValueError, "does not match the pinned cvl1-landing bytes"):
            pinned = model.MECH9_COMMIT
            try:
                model.MECH9_COMMIT = model.MECH8_COMMIT
                model.mech9_master_table(REPO)
            finally:
                model.MECH9_COMMIT = pinned

    @needs(_REPO)
    def test_mech9_landing_appends_entries_311_to_313_below_the_csh1_trailer(self):
        model = self.model()
        lines, before = model.mech9_corrections(REPO)
        # docs/CORRECTIONS.md is append-only from the crt1 / csh1 landing's pin: every line above its closing trailer is
        # unchanged, and entries 311-313 follow it in numeric order (312 and 313 written over their reserved stubs), closed
        # by the reservation commit's trailer "Next free number after the reservations".
        self.assertEqual((len(before), len(lines)), (41924, 42584))
        self.assertEqual(lines[:len(before) - 1], before[:-1])
        self.assertTrue(before[-1].startswith("Next free number: **311**."))
        self.assertEqual(lines[-1], "Next free number after the reservations: **314**.")
        self.assertEqual([line.split(".")[0] for line in lines[len(before) - 1:] if line.startswith("## ")],
                         [f"## {n}" for n in range(311, 314)])
        self.assertFalse(any(line.startswith("## 312. RESERVED") for line in lines), "312 is written in place over its stub")
        with self.assertRaisesRegex(ValueError, "does not match the pinned bytes"):
            pinned = model.MECH9_CORRECTIONS_SHA256
            try:
                model.MECH9_CORRECTIONS_SHA256 = model.MECH8_CORRECTIONS_SHA256
                model.mech9_corrections(REPO)
            finally:
                model.MECH9_CORRECTIONS_SHA256 = pinned
    ...
    def test_cvl1_owns_16_val_split_rows_and_16_two_axis_rows(self):
        ...
        w4 = [row for row in cvl1 if row["arm"].endswith("W4")]
        self.assertEqual((len(w1), len(w4)), (16, 16))
        # W1 (wd 0.1): one kind, the held-out validation split, witnessed by the run's own VAL_SPLIT line.
        self.assertFalse(any(row["argsDeviation"] for row in w1))
        self.assertTrue(all(row["intervention"] == "VAL_SPLIT=5000:302" and row["witness"].startswith("VAL_SPLIT: on dataset=CIFAR10 n_val=5000 n_train=45000 classes=10 per_class=500 split_seed=302 ") for row in w1))
        self.assertEqual({row["experimentId"] for row in cvl1}, {"MT243"})
        # W4 (wd 5e-4): TWO-AXIS (CORRECTIONS 284's rule), listed by the ARGS witness with the VAL_SPLIT clause held beside it.
        self.assertTrue(all(row["argsDeviation"] for row in w4))
        self.assertTrue(all(row["witness"] == "ARGS_WD_BASE: weight-decay-base=5e-4" and row["intervention"] == "--weight-decay-base 5e-4 + VAL_SPLIT=5000:302" for row in w4))
        self.assertTrue(all(model.args_extra_kinds(row["intervention"]) == [("VAL_SPLIT", "5000:302")] and not model.args_extra_args(row["intervention"], row["witness"]) for row in w4))
        counts = {}
        for row in cvl1:
            counts[row["arm"]] = counts.get(row["arm"], 0) + 1
        ...
        for wrong in ("5000:303", "4999:302", "5000", "a:b"):
            with self.assertRaises(ValueError):
                model.additional_witness([line], "VAL_SPLIT", wrong)
        with self.assertRaisesRegex(ValueError, "does not match the pinned bytes"):
            pinned = model.MECH9_INTERVENTIONS_TSV_SHA256
            try:
                model.MECH9_INTERVENTIONS_TSV_SHA256 = model.MECH8_INTERVENTIONS_TSV_SHA256
                model.intervened_runs(REPO)
            finally:
                model.MECH9_INTERVENTIONS_TSV_SHA256 = pinned
