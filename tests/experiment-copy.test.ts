...
test('a newly imported MASTER-TABLE row cannot be published without editorial copy',async()=>{
 ...
 assert.deepEqual(issues.filter(issue=>issue.startsWith('MT228:')),['MT228: public copy is missing from content/experiment-copy.json.']);
});
...
test('the publication gate rejects stale, incomplete, and scorer-style editorial content',async()=>{
 ...
 const issues=module.validateExperimentCopy([...published.experiments,future],{
  ...copy,
  MT228:{question:'Does the next intervention separate the two causal routes?',result:'CO-PRIMARIES P_ROUTE = 49.6327 - 19.3780 = +30.2547 pp'},
  MT999:{question:'Stale copy',result:'Stale result'},
 });
 assert.ok(issues.includes('MT228: result contains register/scorer notation.'));
 assert.ok(issues.includes('MT228: explanation is required for imported records.'));
 assert.ok(issues.includes('MT228: comparison is required for imported records.'));
 assert.ok(issues.includes('MT228: why is required for imported records.'));
...
test('the publication gate scans every reader-facing field for internal notation',async()=>{
 ...
 const future:typeof published.experiments[number]={...byId.get('MT227')!,id:'MT228'};
 const issues=module.validateExperimentCopy([...published.experiments,future],{
  ...copy,
  MT228:{
 ...
 for(const field of ['question','result','explanation','comparison','why']){
  assert.ok(issues.includes(`MT228: ${field} contains register/scorer notation.`),`${field}: ${issues.join(' | ')}`);
 }
});
...
test('the editorial-copy check is a required build step',()=>{
 ...
 assert.match(result.stdout,/Experiment copy check passed: 54 editorial records; all 53 required MASTER-TABLE records \(MT175–MT227\) are covered\./);
});
