...
test('the publication gate rejects stale, incomplete, and scorer-style editorial content',async()=>{
 ...
 const issues=module.validateExperimentCopy([...published.experiments,future],{
  ...copy,
  MT245:{question:'Does the next intervention separate the two causal routes?',result:'CO-PRIMARIES P_ROUTE = 49.6327 - 19.3780 = +30.2547 pp'},
  MT999:{question:'Stale copy',result:'Stale result'},
 });
 assert.ok(issues.includes('MT245: result contains register/scorer notation.'));
 assert.ok(issues.includes('MT245: explanation is required for imported records.'));
 assert.ok(issues.includes('MT245: comparison is required for imported records.'));
 assert.ok(issues.includes('MT245: why is required for imported records.'));
...
test('the publication gate scans every reader-facing field for internal notation',async()=>{
 ...
 const future:typeof published.experiments[number]={...byId.get('MT227')!,id:'MT245'};
 const issues=module.validateExperimentCopy([...published.experiments,future],{
  ...copy,
  MT245:{
 ...
 for(const field of ['question','result','explanation','comparison','why']){
  assert.ok(issues.includes(`MT245: ${field} contains register/scorer notation.`),`${field}: ${issues.join(' | ')}`);
 }
});
