...
test('a newly imported MASTER-TABLE row cannot be published without editorial copy',async()=>{
 ...
 assert.deepEqual(issues.filter(issue=>issue.startsWith('MT239:')),['MT239: public copy is missing from content/experiment-copy.json.']);
});
...
test('the publication gate rejects stale, incomplete, and scorer-style editorial content',async()=>{
 ...
 const issues=module.validateExperimentCopy([...published.experiments,future],{
  ...copy,
  MT239:{question:'Does the next intervention separate the two causal routes?',result:'CO-PRIMARIES P_ROUTE = 49.6327 - 19.3780 = +30.2547 pp'},
  MT999:{question:'Stale copy',result:'Stale result'},
 });
 assert.ok(issues.includes('MT239: result contains register/scorer notation.'));
 assert.ok(issues.includes('MT239: explanation is required for imported records.'));
 assert.ok(issues.includes('MT239: comparison is required for imported records.'));
 assert.ok(issues.includes('MT239: why is required for imported records.'));
...
test('the publication gate scans every reader-facing field for internal notation',async()=>{
 ...
 const future:typeof published.experiments[number]={...byId.get('MT227')!,id:'MT239'};
 const issues=module.validateExperimentCopy([...published.experiments,future],{
  ...copy,
  MT239:{
 ...
 for(const field of ['question','result','explanation','comparison','why']){
  assert.ok(issues.includes(`MT239: ${field} contains register/scorer notation.`),`${field}: ${issues.join(' | ')}`);
 }
});
...
test('the editorial-copy check is a required build step',()=>{
 ...
 assert.match(result.stdout,/Experiment copy check passed: 65 editorial records; all 64 required MASTER-TABLE records \(MT175–MT238\) are covered\./);
});
