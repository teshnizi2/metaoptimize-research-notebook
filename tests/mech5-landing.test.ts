...
test('MT236 carries the outcome the documented rules give it, with every FINAL token quoted', () => {
  ...
  assert.deepEqual(data.experiments.slice(-8, -7).map(e => e.id), ['MT236'], 'appended before the later cwd4, cwd5, caw2, cgw1, crt1, csh1 and cvl1 rows');
...
test('the 15 runs link to MT236 with sanitized logs, and exactly the 12 masked runs are marked', () => {
  ...
  assert.equal(runs.filter(run => run.batch !== 'cvl1' && run.parameters.intervention).length, 183, 'the 153 earlier patch interventions, these 12, and cwd4\'s 18');
