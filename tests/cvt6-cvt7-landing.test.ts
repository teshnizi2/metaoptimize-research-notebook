...
test('MT224 (cvt6) is Mixed and MT225 (cvt7) Goal met under the documented rules, with every FINAL token quoted', () => {
  assert.deepEqual(data.experiments.slice(-2).map(e => e.id), ['MT224', 'MT225'], 'appended after every existing record, in line order');
  assert.deepEqual([data.meta.stats.experiments, data.meta.stats.researchQuestions, data.meta.stats.methodChecks, data.meta.stats.runs], [162, 144, 18, 3085]);
...
test('the 36 runs link to MT224 / MT225 with sanitized logs; the 24 intervened runs are marked, two-kind runs with both witnesses', () => {
  ...
  assert.equal(runs.filter(run => run.parameters.intervention).length, 75, 'cvt1 9 + cvt3 9 + cvt2 15 + cvt4 12 + cvt5 6 + cvt6 15 + cvt7 9');
