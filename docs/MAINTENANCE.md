...
Use the experiment IDs shown in the experiment register. The current snapshot contains 156 IDs (138 research questions and 18 method checks). The command validates against the actual imported register, including `CVK2`, rather than a hard-coded count.
...
- `ADDED_PHASES` adds research phases the campaign's `research_timeline.csv` does not have, in the same format (currently phase-09, 15-16 Sep 2026, linked to MT212-MT219). The timeline CSV itself is published unchanged.
