...
Use the experiment IDs shown in the experiment register. The current snapshot contains 158 IDs (140 research questions and 18 method checks). The command validates against the actual imported register, including `CVK2`, rather than a hard-coded count.
...
- `ADDED_PHASES` adds research phases the campaign's `research_timeline.csv` does not have, in the same format (currently phase-09, 15-16 Sep 2026, linked to MT212-MT221). The timeline CSV itself is published unchanged.
...
- All 3,019 runs in this snapshot have a sanitized raw log linked by `logHref`, with original/public SHA-256 pairs. If a future snapshot omits an archived log, retain its explicit archived-but-unpublished availability label rather than implying that no log exists.
